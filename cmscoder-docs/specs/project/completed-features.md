...
| Feature 2：统一模型接入 | 未开始 | 依赖 Feature 1 完成 |
...
| 安全存储 + 本地缓存 | `lib/storage.js` | 已完成（macOS Keychain / Windows AES-256-GCM / Linux libsecret / file fallback） |
...
| user-service | Session revoke | 已完成 |
...
1. **联调**：通过插件端发起登录，验证登录链路
2. **OpenCode hooks**：确认 OpenCode 版本和扩展机制后补充
3. **模型接入**：实现 Claude Code / OpenCode 的模型 endpoint 重定向
